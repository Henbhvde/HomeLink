  ...
  return (
    <div className="workspace-shell manager-gold-shell min-h-screen text-cream">
      ...
      {isHelpOpen && createPortal(
        <div className="fixed inset-0 z-[115] flex items-center justify-center overflow-hidden bg-black/65 p-3 backdrop-blur-sm sm:p-4" role="dialog" aria-modal="true" aria-labelledby="help-title" onClick={() => setIsHelpOpen(false)}>
          <section className="max-h-[90vh] w-full max-w-[540px] overflow-y-auto overscroll-contain rounded-2xl border border-sand/25 bg-[#2b2926] p-4 shadow-2xl sm:p-5" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="mt-4 space-y-1.5">
              ...
              ].map(([title, description]) => <div key={title} className="rounded-xl border border-sand/15 bg-white/[.035] px-3.5 py-2.5"><b className="text-xs text-cream">{title}</b><p className="mt-0.5 text-[11px] leading-4 text-sand-400">{description}</p></div>)}
            </div>
            <div className="mt-4 flex justify-end"><button type="button" onClick={() => setIsHelpOpen(false)} className="rounded-xl border border-sand/25 px-4 py-2 text-xs font-semibold text-sand hover:bg-sand/[.08]">Хаах</button></div>
          </section>
        </div>,
        document.body,
      )}
