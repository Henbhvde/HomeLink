    ...
    >
      ...
      {isRunDialogOpen && createPortal(<div className="fixed inset-0 z-50 flex min-h-screen items-center justify-center overflow-y-auto bg-black/75 p-5 backdrop-blur-sm"><div className="accountant-modal-panel my-auto max-h-[calc(100vh-2rem)] w-full max-w-md overflow-y-auto rounded-2xl border border-white/10 bg-[#171614] p-6 shadow-2xl">
        ...
        {runStatus === 'review' && <><div className="mt-6 space-y-4 rounded-xl border border-white/8 bg-black/20 p-4 text-sm">
          ...
          <div><div className="mb-2 flex items-end justify-between gap-3"><label className="text-xs text-sand-300">Хамрах айлын тоо<Input type="number" min="0" max={billingUnits.length} value={selectedUnitIds.length} onChange={(event) => { const count = Math.max(0, Math.min(billingUnits.length, Number.parseInt(event.target.value, 10) || 0)); setSelectedUnitIds(billingUnits.slice(0, count).map((unit) => unit.id)); }} className="mt-1 h-9 w-28" /></label><button type="button" className="pb-2 text-xs font-semibold text-sand" onClick={() => setSelectedUnitIds(selectedUnitIds.length === billingUnits.length ? [] : billingUnits.map((unit) => unit.id))}>{selectedUnitIds.length === billingUnits.length ? 'Бүгдийг болих' : 'Бүгдийг сонгох'}</button></div>
