    ...
    >
      ...
      {isRunDialogOpen && createPortal(<div className="fixed inset-0 z-50 flex min-h-screen items-center justify-center overflow-y-auto bg-black/75 p-5 backdrop-blur-sm"><div className="accountant-modal-panel my-auto max-h-[calc(100vh-2rem)] w-full max-w-md overflow-y-auto rounded-2xl border border-white/10 bg-[#171614] p-6 shadow-2xl">
        ...
        {runStatus === 'review' && <><div className="mt-6 space-y-4 rounded-xl border border-white/8 bg-black/20 p-4 text-sm">
          <div className="grid grid-cols-2 gap-3"><label className="text-xs text-sand-300">Эхлэх огноо<Input type="date" value={periodStart} onChange={(event) => setPeriodStart(event.target.value)} className="mt-1" /></label><label className="text-xs text-sand-300">Дуусах огноо<Input type="date" value={periodEnd} onChange={(event) => setPeriodEnd(event.target.value)} className="mt-1" /></label></div>
          ...
          <div><div className="mb-2 flex items-end justify-between gap-3"><label className="text-xs text-sand-300">Хамрах айлын тоо<Input type="number" min="0" value={householdCount} onChange={(event) => { const value = event.target.value.replace(/[^0-9]/g, ''); setHouseholdCount(value); const count = Number.parseInt(value, 10) || 0; setSelectedUnitIds(billingUnits.slice(0, count).map((unit) => unit.id)); }} className="mt-1 h-9 w-28" /></label><button type="button" className="pb-2 text-xs font-semibold text-sand" onClick={() => { const nextIds = selectedUnitIds.length === billingUnits.length ? [] : billingUnits.map((unit) => unit.id); setSelectedUnitIds(nextIds); setHouseholdCount(String(nextIds.length)); }}>{selectedUnitIds.length === billingUnits.length ? 'Бүгдийг болих' : 'Бүгдийг сонгох'}</button></div>
            <div className="max-h-40 space-y-1 overflow-y-auto rounded-lg border border-white/8 p-2">{unitsLoading ? <p className="p-2 text-xs text-sand-400">Айлын мэдээлэл уншиж байна...</p> : billingUnits.map((unit) => <label key={unit.id} className="flex cursor-pointer items-center gap-2 rounded-md p-2 text-xs text-sand-200 hover:bg-white/5"><input type="checkbox" checked={selectedUnitIds.includes(unit.id)} onChange={() => setSelectedUnitIds((current) => current.includes(unit.id) ? current.filter((id) => id !== unit.id) : [...current, unit.id])} /><span>{unit.building} · {unit.entrance} · {unit.floor}-р давхар · {unit.number}</span></label>)}</div>
            <p className="mt-2 text-right text-xs text-sand-400">Сонгосон: {selectedUnitIds.length} / {billingUnits.length}</p>
          </div>
