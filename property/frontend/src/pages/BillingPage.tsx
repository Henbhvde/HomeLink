  ...
  const openInvoiceRun = async () => {
    ...
    if (!invoiceRunCreated && token && user?.workspace?.id) {
      ...
      try {
        ...
        const units = Array.isArray(payload?.data) ? (payload.data as BillingUnit[]).filter((unit) => unit.hasActiveResident) : [];
